...
double fmod(double dummy0, double dummy1);
...
class mydsp : public dsp {
 ...
 public:
	...
	virtual void instanceResetUserInterface() {
		fEntry0 = FAUSTFLOAT(0.040000000000000001);
		fEntry1 = FAUSTFLOAT(0.0);
		fEntry2 = FAUSTFLOAT(0.01);
	...
	virtual void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->addNumEntry("winlength_secs", &fEntry0, 0.040000000000000001, 0.001, 0.25, 0.0001);
		ui_interface->declare(&fEntry2, "2", "");
		ui_interface->addNumEntry("xfade_secs", &fEntry2, 0.01, 0.0001, 0.25, 0.0001);
		ui_interface->declare(&fEntry1, "3", "");
		ui_interface->addNumEntry("shift", &fEntry1, 0.0, -12.0, 12.0, 0.01);
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		double fSlow1 = pow(2.0, (0.083333333333333329 * double(fEntry1)));
		double fSlow2 = (1.0 / double(int((fConst0 * double(fEntry2)))));
		for (int i = 0; (i < count); i = (i + 1)) {
			double fTemp0 = double(input0[i]);
			fVec0[(IOTA & 65535)] = fTemp0;
			fRec0[0] = fmod((fSlow0 + ((fRec0[1] + 1.0) - fSlow1)), fSlow0);
			int iTemp1 = int(fRec0[0]);
			double fTemp2 = floor(fRec0[0]);
			double fTemp3 = (1.0 - fRec0[0]);
			double fTemp4 = min((fSlow2 * fRec0[0]), 1.0);
			double fTemp5 = (fSlow0 + fRec0[0]);
			int iTemp6 = int(fTemp5);
			double fTemp7 = floor(fTemp5);
			output0[i] = FAUSTFLOAT(((((fVec0[((IOTA - (iTemp1 & 65535)) & 65535)] * (fTemp2 + fTemp3)) + ((fRec0[0] - fTemp2) * fVec0[((IOTA - ((iTemp1 + 1) & 65535)) & 65535)])) * fTemp4) + (((fVec0[((IOTA - (iTemp6 & 65535)) & 65535)] * ((fTemp7 + fTemp3) - fSlow0)) + ((fSlow0 + (fRec0[0] - fTemp7)) * fVec0[((IOTA - ((iTemp6 + 1) & 65535)) & 65535)])) * (1.0 - fTemp4))));
			IOTA = (IOTA + 1);
			fRec0[1] = fRec0[0];
	...
	FAUST_ADDNUMENTRY("winlength_secs", fEntry0, 0.040000000000000001, 0.001, 0.25, 0.0001);
	FAUST_ADDNUMENTRY("xfade_secs", fEntry2, 0.01, 0.0001, 0.25, 0.0001);
	FAUST_ADDNUMENTRY("shift", fEntry1, 0.0, -12.0, 12.0, 0.01);
#endif
